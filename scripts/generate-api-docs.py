...
def generate_curl_from_generic(generic_cli: str) -> str:
    ...
    method_match = re.search(
        r"(GET|POST|PUT|DELETE|PATCH)\s+(/api/[^\s\']+)", generic_cli
    )
    if not method_match:
...
def generate_endpoint_section(
    path: str, endpoint_info: Dict, examples: List[Dict]
) -> str:
...
if __name__ == "__main__":
    ...
    endpoint_examples = map_examples_to_endpoints(
        cli_examples, curl_examples, endpoints
    )
